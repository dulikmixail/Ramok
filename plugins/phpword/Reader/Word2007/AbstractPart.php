    ...
    protected function readRun(XMLReader $xmlReader, \DOMElement $domNode, $parent, $docPart, $paragraphStyle = null)
    {
        ...
        if ($domNode->nodeName == 'w:hyperlink') {
            ...
            // Image
				...
				//добавляем картинку в обьект
			//	var_dump($parent);
				$rId = $xmlReader->getAttribute('r:embed',$domNode,'w:drawing/wp:anchor/a:graphic/a:graphicData/pic:pic/pic:blipFill/a:blip'); //путь в dom дереве
							
				$target = $this->getMediaTarget($docPart, $rId);
				$imageSource = "zip://{$this->docFile}#{$target}";	
					
				 if (!is_null($target)) {	
					$parent->addImage($imageSource); //добавляем обьект картинку
				 }
