...
// PHPC Control Panel Plugin (40.5) - PHPC Control Center v1.0  by Antylevsky Aleksei (Next)
...
if($database->isTablePresent("styles")) {
  ...
  if(PhpcLocale=="")
    makeMenuItem("phpc_languages","phpc.php?action=modifylanguages");
  makeMenuGroup("footer");
...
else makeMenuItem("admin_install","phpc.php?action=install");
makeMenuGroup("footer");
...
define("SEARCH_INHERITED",0);
define("SEARCH_CURRENT_SET",1);
define("SEARCH_ALL_SETS",2);
...
$infoTemplates=
  ...
  "content:phpeditor:phpc_addedittemplate_content,".
...
<var:language:phpc_deftemplate_general nofilter>
...
<var:language:phpc_deftemplate_index nofilter>
...
<var:language:phpc_deftemplate_404 nofilter>
...
<html>
<head>
<title><var:language:phpc_deftemplate_standarderror1 nofilter></title>
...
<div class="link"><a href="javascript:history.back(1)"><var:language:phpc_deftemplate_standarderror2 nofilter></a></div>
...
<title><var:language:phpc_deftemplate_standardredirect1 nofilter></title>
<insert:htmlStandardStyles/>
</head>
...
<div class="message"><var:message></div>
<div class="link"><a href="<var:link>"><var:language:phpc_deftemplate_standardredirect2 nofilter></a></div>
...
function prefixMatch($text, $prefix)
{
  return !phpcstrncasecmp($text,$prefix,strlen($prefix));
}

function groupingTreeSortCallback($value1, $value2)
{
  return -phpcstrcasecmp($value1["prefix"],$value2["prefix"]);
}
...
function createInheritanceGroupingTree($tableGroups, $tableItems, $setChain, $defaultTitleOnce, $defaultTitleMulti)
{
  global $database;
  $sets=array();
  foreach($setChain as $setid)
    $sets[]=$database->customQuery("SELECT id,name FROM $tableItems WHERE setid=$setid ORDER BY name");
  $items=array();
  foreach($sets as $setIndex=>$set) foreach($set as $item) {
    $name=phpcstrtolower($item["name"]);
    if(!isset($items[$name])) {
    ...
    else $items[$name]["inheritance"]="inherited";
  }
  ksort($items);
  $result=$database->getOrderedLines($tableGroups,"displayorder");
  uasort($result,"groupingTreeSortCallback");
  foreach($result as $groupIndex=>$group) {
    $prefix=$group["prefix"];
    $result[$groupIndex]["items"]=array();
    foreach($items as $itemKey=>$item) if(prefixMatch($item["name"],$prefix)) {
      $result[$groupIndex]["items"][$itemKey]=$item;
      unset($items[$itemKey]);
    }
  }
  ksort($result);
  $title=count($result)?$defaultTitleMulti:$defaultTitleOnce;
  $result[]=array("id"=>0,"prefix"=>"","title"=>$title,"items"=>$items);
  return $result;
}

function createSimpleGroupingTree($tableGroups, $tableItems, $defaultTitleOnce, $defaultTitleMulti)
{
  global $database;
  $source=$database->getOrderedLines($tableItems,"name");
  $items=array();
  foreach($source as $item) $items[phpcstrtolower($item["name"])]=$item;
  ksort($items);
  $result=$database->getOrderedLines($tableGroups,"displayorder");
  uasort($result,"groupingTreeSortCallback");
  foreach($result as $groupIndex=>$group) {
...
function searchGroup($table, $name, $strict=false)
{
  global $database;
  $groups=$database->getOrderedLines($table,"displayorder");
  foreach($groups as $group) {
    $prefix=$group["prefix"];
    if($strict && !phpcstrcasecmp($name,$prefix)) return $group["id"];
    if(!$strict && prefixMatch($name,$prefix)) return $group["id"];
  }
  return 0;
}
...
function processSearch($subject, $pattern, $area, $grouping=false, $defaultTitleOnce="", $defaultTitleMulti="")
{
  global $database;
  $items=array();
  if($area==SEARCH_INHERITED) {
    $setChain=createInheritanceChain($subject);
    $sets=array();
    foreach($setChain as $setid)
      $sets[]=$database->getOrderedLines("{$subject}s","name","setid=$setid");
    $items=array();
    foreach($sets as $setIndex=>$set) foreach($set as $item) {
      $name=phpcstrtolower($item["name"]);
      if(!isset($items[$name])) {
  ...
  else if($area==SEARCH_CURRENT_SET) {
    $style=getAdminStyle();
    $setid=$style["{$subject}setid"];
    $source=$database->getOrderedLines("{$subject}s","name","setid=$setid");
    foreach($source as $item) {
      $name=phpcstrtolower($item["name"]);
      $items[$name]=$item;
    }
  }
  else if($area==SEARCH_ALL_SETS) {
    $source=$database->getOrderedLines("{$subject}s","name");
    foreach($source as $item) {
  ...
  foreach($items as $name=>$item) {
    ...
    if($count) $items[$name]["fragments"]=$count; else unset($items[$name]);
  }
  ksort($items);
  if(!$grouping) return $items;
  $result=$database->getOrderedLines("{$subject}groups","displayorder");
  foreach($result as $groupIndex=>$group) {
    $prefix=$group["prefix"];
    $result[$groupIndex]["items"]=array();
    foreach($items as $itemKey=>$item) if(prefixMatch($item["name"],$prefix)) {
    ...
    if(!count($result[$groupIndex]["items"])) unset($result[$groupIndex]);
  }
  $title=count($result)?$defaultTitleMulti:$defaultTitleOnce;
  if(count($items)) $result[]=array("id"=>0,"prefix"=>"","title"=>$title,"items"=>$items);
  return $result;
}
  ...
  if($database->isTablePresent("styles")) {
    ...
    switch($database->type) {
      ...
      case "dagsql":
        ...
        $database->customQuery("CREATE TABLE pages (".
          ...
          "title$locale TINYTEXT NOT NULL,".
        ...
        $database->customQuery("CREATE TABLE cache (".
          "id INT NOT NULL AUTO_INCREMENT,".
        ...
        makeNotification("admin_installtable","cache");
    ...
    if($success) {
      ...
      foreach($sampleTemplates as $name=>$content) {
        $parent=($name=="index" || $name=="404")?"general":"";
        installTemplate($name,$content,$parent);
      }
      foreach($sampleBundles as $name=>$content) installBundle($name,$content);
...
if($action=="statistics") {
  ...
  $totalOthers=0;
  foreach($tablesOthers as $table)
    $totalOthers+=$database->getLinesCount($table);
  ...
  $total=$totalPages+$totalTemplates+$totalBundles+
    $totalReplacements+$totalOthers+$totalStyles;
  ...
  makeTableCellSimple($conclusion,array("colspan"=>2,"align"=>"center"));
  makeTable("footer");
}
...
if($action=="export") {
  ...
  if($database->isLinePresent("styles","foradmin=1")) {
    ...
    $templates=createInheritanceArray("templates",$templateSetChain);
    $bundles=createInheritanceArray("bundles",$bundleSetChain);
...
if($action=="docheck") {
  ...
  $pages=$database->getOrderedLines("pages","name","visible=1");
  ...
  foreach($templatesets as $set) {
    ...
    $templates=array();
    foreach($setChain as $setid) {
      $items=$database->customQuery("SELECT name,parent FROM templates WHERE setid=$setid");
      foreach($items as $item) {
        $name=phpcstrtolower($item["name"]);
        if(!isset($templates[$name])) $templates[$name]=$item;
      }
    }
    ksort($templates);
  ...
  foreach($bundlesets as $set) {
    ...
    $setsList=implode(",",$setChain);
    $bundles=$database->customQuery("SELECT DISTINCT name FROM bundles WHERE setid IN($setsList) ORDER BY name");
    $bundles=array_change_key_case(extractArrayColumns($bundles,"name","name"));
    foreach($pages as $page) {
      $bundleList=explodeSmart(",",$page["bundles"]);
      foreach($bundleList as $bundle) if(!isset($bundles[phpcstrtolower($bundle)])) {
...
if($action=="doaddpage" || $action=="doeditpage") {
  ...
  makeBreak();
  sendParameter("expand",searchGroup("pagegroups",$name));
...
if($action=="doaddpagegroup" || $action=="doeditpagegroup") {
  ...
  makeBreak();
  sendParameter("expand",searchGroup("pagegroups",$prefix,true));
  if($success) $action="modifypages";
}
...
if($action=="modifypages") {
  ...
  $groups=createSimpleGroupingTree("pagegroups","pages",
    $language["phpc_modifypages_defgrouponce"],
    $language["phpc_modifypages_defgroupmulti"]);
  makeTree("header","phpc_modifypages_tree");
  foreach($groups as $group) {
    $expand=$group["id"]==(int)$expandGroup || $expandAll;
    $links=array();
    if(!$expand) $links+=array(
      "phpc_modifypages_expand"=>"phpc.php?action=modifypages&expand=$group[id]",
      "phpc_modifypages_expandall"=>"phpc.php?action=modifypages&expand=all");
    if($group["id"]) $links+=array(
      "phpc_modifypages_groupedit"=>"phpc.php?action=editpagegroup&id=$group[id]",
      "phpc_modifypages_groupremove"=>"phpc.php?action=removepagegroup&id=$group[id]",
      "phpc_modifypages_groupadd"=>"phpc.php?action=addpage&name=".urlencode($group["prefix"]));
    if(!$group["id"]) $links+=array(
  ...
  if(count($groups)>1) {
    ...
    foreach($groups as $group) if($group["id"])
...
if($action=="doaddpair") {
  ...
  makeBreak();
  sendParameter("expand",searchGroup("templategroups",$template));
...
if(in_array($action,explode(",",$actions))) {
  ...
  makeBreak();
  sendParameter("expand",searchGroup("templategroups",$name));
...
if($action=="viewtemplate") {
  ...
  $content=$syntaxColorer->processPhpcTemplate($template["content"]);
  makeNotification("phpc_viewtemplate_prompt",$template["name"]);
  makeFormattedText($content);
  makeLinks(array("phpc_viewtemplate_execute"=>"_blank:phpc.php?action=executetemplate&id=$id"));
}
...
if($action=="doaddtemplategroup" || $action=="doedittemplategroup") {
  ...
  makeBreak();
  sendParameter("expand",searchGroup("templategroups",$prefix,true));
...
if($action=="modifytemplates") {
  ...
  $groups=createInheritanceGroupingTree("templategroups","templates",$setChain,
    $language["phpc_modifytemplates_defgrouponce"],
    $language["phpc_modifytemplates_defgroupmulti"]);
  makeTree("header","phpc_modifytemplates_tree");
  foreach($groups as $group) {
    $expand=$group["id"]==(int)$expandGroup || $expandAll;
    $links=array();
    if(!$expand) $links+=array(
      "phpc_modifytemplates_expand"=>"phpc.php?action=modifytemplates&expand=$group[id]",
      "phpc_modifytemplates_expandall"=>"phpc.php?action=modifytemplates&expand=all");
    if($group["id"]) $links+=array(
      "phpc_modifytemplates_groupedit"=>"phpc.php?action=edittemplategroup&id=$group[id]",
      "phpc_modifytemplates_groupremove"=>"phpc.php?action=removetemplategroup&id=$group[id]",
      "phpc_modifytemplates_groupadd"=>"phpc.php?action=addtemplate&name=".urlencode($group["prefix"]));
    if(!$group["id"]) $links+=array(
...
if(in_array($action,explode(",",$actions))) {
  ...
  makeBreak();
  sendParameter("expand",searchGroup("bundlegroups",$name));
...
if($action=="doaddbundlegroup" || $action=="doeditbundlegroup") {
  ...
  makeBreak();
  sendParameter("expand",searchGroup("bundlegroups",$prefix,true));
...
if($action=="modifybundles") {
  ...
  $groups=createInheritanceGroupingTree("bundlegroups","bundles",$setChain,
    $language["phpc_modifybundles_defgrouponce"],
    $language["phpc_modifybundles_defgroupmulti"]);
  makeTree("header","phpc_modifybundles_tree");
  foreach($groups as $group) {
    $expand=$group["id"]==(int)$expandGroup || $expandAll;
    $links=array();
    if(!$expand) $links+=array(
      "phpc_modifybundles_expand"=>"phpc.php?action=modifybundles&expand=$group[id]",
      "phpc_modifybundles_expandall"=>"phpc.php?action=modifybundles&expand=all");
    if($group["id"]) $links+=array(
      "phpc_modifybundles_groupedit"=>"phpc.php?action=editbundlegroup&id=$group[id]",
      "phpc_modifybundles_groupremove"=>"phpc.php?action=removebundlegroup&id=$group[id]",
      "phpc_modifybundles_groupadd"=>"phpc.php?action=addbundle&name=".urlencode($group["prefix"]));
    if(!$group["id"]) $links+=array(
...
if($action=="dosearchtemplates" || $action=="dosearchbundles") {
  ...
  if($text!="") {
    ...
    $groups=processSearch($subject,$pattern,SEARCH_INHERITED,true,
      $language["phpc_modify{$subject}s_defgrouponce"],
      $language["phpc_modify{$subject}s_defgroupmulti"]);
    if(count($groups)) {
...
if($action=="doreplacetemplates" || $action=="doreplacebundles") {
  ...
  if($text!="") {
    $pattern=createSearchPattern($text,$case,$words,$regexp);
    $area=$allsets?SEARCH_ALL_SETS:SEARCH_CURRENT_SET;
...
if($action=="modifyreplacements") {
  ...
  $items=createInheritanceArray("replacements",$setChain);
...
if($action=="assignstyles") {
  ...
  $values1=extractArrayColumns($styles1,"id","title");
  $values2=extractArrayColumns($styles2,"id","title");
  ...
  makeFormChooser("phpc_assignstyles_forusers","","id1",$id1,$values1);
  makeFormChooser("phpc_assignstyles_foradmin","","id2",$id2,$values2);
  makeForm("footer");
}
